...
type OnChange<T> = <K extends keyof T>(
  propName: K,
  cb: Callback<T, K>,
  triggerOnBind?: boolean
) => () => void
...
const observableState = <T extends {}>(defaultState: T): ObservableState<T> => {
  ...
  const extendedState = {
    ...
    trigger: <K extends keyof T>(propName: K) => {
      for (const cb of cbMap.get(propName) || [])
        cb(proxy[propName], proxy[propName])
    },
    ...defaultState,
  }
